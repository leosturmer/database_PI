...
from textual.widgets import (Button, Input, TextArea, Footer, Header,
                             Label, Static, MaskedInput, OptionList, Select, SelectionList, TabbedContent, TabPane, DataTable, Collapsible, Switch, ContentSwitcher)
...
class TelaProdutos(Screen):
    ...
    def compose(self):
        ...
        with HorizontalGroup():
            yield Button('Cadastrar produtos', id='tab_cadastro')
            yield Button('Atualizar produtos', id='tab_atualizar')

        with ContentSwitcher(initial='tab_cadastro'):
            yield ContainerProdutos(id='tab_cadastro')
            with VerticalGroup(id='tab_atualizar'):
                with HorizontalGroup():
